...
try {
  provider = new ethers.providers.JsonRpcProvider(alchemyApi);
} catch (error) {
  console.error("Failed to initialize provider. Please check your Alchemy API URL.");
  process.exit(1); // Exit the script if provider initialization fails
}
...
async function checkAndSend() {
  try {
    ...
    console.error("Error in checkAndSend:", error.message);
  }
}
